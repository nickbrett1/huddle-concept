...
# "huddle-concept-dev" (the repo name plus the "-dev"
...
# than adding a second one - a stale "huddle-concept-dev" is overwritten by
...
PROJECT_NAME="huddle-concept"
AGENT_NAME="huddle-concept-dev"
WORKSPACE="/workspaces/huddle-concept"
...
if [ -x "/workspaces/huddle-concept/scripts/agent-dev.sh" ]; then
  "/workspaces/huddle-concept/scripts/agent-dev.sh" start >/dev/null 2>&1 || true
fi
POST_START_HOOK
}
